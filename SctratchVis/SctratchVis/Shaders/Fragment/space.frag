...
{
	float halfSz = size * 0.5;
	float c = floor((p = halfSz) / size);
	p = mod(p + halfSz, size) - halfSz;
...
// the derivative of the path function
...
// the second derivative of the path function
// used top generate tilt
...
vec3 weird(vec2 p)
{
	...
	col += hsv2rgb(vec3(fract(-0.1 * uTime + 0.125 * a + 0.5 * v.x + 0.125 * w.x), abs(0.5 + tanh_approx(v.y * w.y)), tanh_approx(0.1 + abs(v.y - w.y))));
...
vec3 effect(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p)
{
	float lp = length(p);
	vec2 np = p + 1.0 / uRes.xy;
	...
	vec3 c = alphaBlend(skyCol, acol);

	return c;
}

vec3 postProcess(vec3 c, vec2 q)
{
	c = clamp(c, 0.0, 1.0);
	c = pow(c, 1.0 / std_gamma);
	c = c * 0.6 + 0.4 * c * c * (3.0 - 2.0 * c);
	c = mix(c, vec3(dot(c, vec3(0.33))), -0.4);
	c *= 0.5 + 0.5 * pow(19.0 * q.x * q.y * (1.0 - q.x) * (1.0 - q.y), 0.7);

	return c;
}


vec3 col(vec2 p, vec2 q)
{
	vec3 ret;
	vec3 c = vec3(0.0);
...
void main()
{
	vec2 q = gl_FragCoord.xy / uRes.xy - 1.;
	vec2 p = -1.0 + 2.0 * q;
	p.x *= uRes.x / uRes.y;

	vec4 ret = vec4(col(q, p), 1.0);
